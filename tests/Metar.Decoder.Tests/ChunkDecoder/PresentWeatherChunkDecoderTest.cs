...
namespace Metar.Decoder_tests.chunkdecoder
{
    ...
    public class PresentWeatherChunkDecoderTest
    {
        ...
        public void TestParsePresentWeatherChunk(PresentWeatherChunkDecoderTester chunkToTest)
        {
            ...
            ClassicAssert.That(pw.Count, Is.EqualTo(chunkToTest.NbPhenomens));
            if (chunkToTest.NbPhenomens > 0)
            {
                var phenom1 = pw[0];
                ClassicAssert.That(phenom1.IntensityProximity, Is.EqualTo(chunkToTest.intensity1));
                ClassicAssert.That(phenom1.Characteristics, Is.EqualTo(chunkToTest.carac1));
                ClassicAssert.That(phenom1.Types, Is.EqualTo(chunkToTest.type1));
            }
            if (chunkToTest.NbPhenomens > 1)
            {
                var phenom2 = pw[1];
                ClassicAssert.That(phenom2.Types, Is.EqualTo(chunkToTest.type2));
            }
